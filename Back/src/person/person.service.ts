...
export class PeopleService {
  ...
  async personById(personId: string) {
    const person: Person =
      await this.peopleRepository.personById(personId);
    return person;
  }
  ...
  async personByEmail(email: string): Promise<Person> {
    const person: Person =
      await this.peopleRepository.personByEmail(email);
    return person;
  }

  async personByDni (dni: string): Promise<Person> {
    const person: Person =
      await this.peopleRepository.personByDni(dni);
    return person;
  }
  ...
  async createPerson(personInfo: Partial<Person>) {
    ...
    const personByDniExist: Person = 
      await this.peopleRepository.personByDni(personInfo.dni);
    if (personByDniExist)
