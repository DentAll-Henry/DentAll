...
import { productsDB } from 'src/db/productsDB';
...
=======
    ...
    async seedProducts() {
        ...
        if (p.length === 0) {
            const productsInDB = await Promise.all(products.map(async (product) => {
                const p = await this.productService.postProduct(product)
            }))
